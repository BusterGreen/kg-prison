...
local function JobDone()
    ...
    if math.random(1, 100) <= 50 then
        KGCore.Functions.Notify(Lang:t('success.time_cut'))
...
local function StartWork()
    ...
    KGCore.Functions.Progressbar('work_electric', Lang:t('info.working_electricity'), math.random(5000, 10000), false, true, {
    ...
    for k in pairs(Config.Locations.jobs) do
        for i = 1, #Config.Locations.jobs[k] do
            ...
            if Config.UseTarget then
                exports['kg-target']:AddBoxZone('work_' .. k .. '_' .. i, current.coords.xyz, 1.5, 1.6, {
                    name = 'work_' .. k .. '_' .. i,
                    heading = 12.0,
                    debugPoly = false,
                    minZ = 19,
                ...
                jobZone:onPlayerInOut(function(isPointInside)
                    ...
                    if isInside then
                        exports['kg-core']:DrawText(Lang:t('info.job_interaction'), 'left')
                    else
