KGCore = exports['kg-core']:GetCoreObject() -- Used Globally
inJail = false
jailTime = 0
currentJob = nil
CellsBlip = nil
...
local function ApplyClothes()
	...
	if DoesEntityExist(playerPed) then
		Citizen.CreateThread(function()
			...
			if gender == 0 then
				TriggerEvent('kg-clothing:client:loadOutfit', Config.Uniforms.male)
			else
				TriggerEvent('kg-clothing:client:loadOutfit', Config.Uniforms.female)
			end
		end)
	end
end
...
RegisterNetEvent('KGCore:Client:OnPlayerLoaded', function()
	...
	KGCore.Functions.TriggerCallback('prison:server:IsAlarmActive', function(active)
		if active then
	...
	exports['kg-target']:AddTargetEntity(freedom_ped, {
		options = {
			{
				type = 'client',
...
RegisterNetEvent('prison:client:Enter', function(time)
	...
	if invokingResource and invokingResource ~= 'kg-policejob' and invokingResource ~= 'kg-ambulancejob' and invokingResource ~= GetCurrentResourceName() then
		-- Use KGCore.Debug here for a quick and easy way to print to the console to grab your attention with this message
		KGCore.Debug({ ('Player with source %s tried to execute prison:client:Enter manually or from another resource which is not authorized to call this, invokedResource: %s'):format(GetPlayerServerId(PlayerId()), invokingResource) })
		return
	end

	KGCore.Functions.Notify(Lang:t('error.injail', { Time = time }), 'error')
	...
	Wait(2000)
	DoScreenFadeIn(1000)
	KGCore.Functions.Notify(Lang:t('error.do_some_work', { currentjob = Config.Jobs[currentJob] }), 'error')
end)

RegisterNetEvent('prison:client:Leave', function()
	if jailTime > 0 then
		KGCore.Functions.Notify(Lang:t('info.timeleft', { JAILTIME = jailTime }))
	else
		jailTime = 0
		...
		RemoveBlip(ShopBlip)
		ShopBlip = nil
		KGCore.Functions.Notify(Lang:t('success.free_'))
		DoScreenFadeOut(500)
		while not IsScreenFadedOut() do
			Wait(10)
		end
	...
	while true do
		...
		if jailTime > 0 and inJail then
			...
			if jailTime > 0 and inJail then
				...
				if jailTime <= 0 then
					jailTime = 0
					KGCore.Functions.Notify(Lang:t('success.timesup'), 'success', 10000)
				end
				TriggerServerEvent('prison:server:SetJailStatus', jailTime)
			end
		end
	...
	if not Config.UseTarget then
		...
		freedom:onPlayerInOut(function(isPointInside)
			...
			if isPointInside then
				...
				exports['kg-core']:DrawText('[E] Check Time', 'left')
			else
				exports['kg-core']:HideText()
			end
		end)
