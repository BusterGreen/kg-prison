...
RegisterNetEvent('prison:server:SetJailStatus', function(jailTime)
    ...
    if jailTime > 0 then
        if Player.PlayerData.job.name ~= 'unemployed' then
            ...
            TriggerClientEvent('KGCore:Notify', src, Lang:t('info.lost_job'))
        end
    else
        GotItems[source] = nil
    end
...
RegisterNetEvent('prison:server:GiveJailItems', function(escaped)
    ...
    for _, v in pairs(Player.PlayerData.metadata['jailitems']) do
        exports['kg-inventory']:AddItem(src, v.name, v.amount, false, v.info, 'prison:server:GiveJailItems')
    end
    Player.Functions.SetMetaData('jailitems', {})
end)
...
RegisterNetEvent('prison:server:CheckChance', function()
    ...
    if not exports['kg-inventory']:AddItem(src, 'phone', 1, false, false, 'prison:server:CheckChance') then return end
    TriggerClientEvent('kg-inventory:client:ItemBox', src, KGCore.Shared.Items['phone'], 'add')
    TriggerClientEvent('KGCore:Notify', src, Lang:t('success.found_phone'), 'success')
    GotItems[src] = true
end)

KGCore.Functions.CreateCallback('prison:server:IsAlarmActive', function(_, cb)
    cb(AlarmActivated)
end)
